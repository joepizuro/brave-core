...
std::string ToStr(base::Optional<base::TimeDelta> lifetime) {
  if (!lifetime)
    return "nullopt";
  return std::to_string(lifetime->InSeconds());
}
...
void PermissionLifetimeManager::PermissionDecided(
    ...
    bool is_one_time) {
  if (content_setting != ContentSetting::CONTENT_SETTING_ALLOW || is_one_time) {
    return;
  }
...
void PermissionLifetimeManager::OnExpirationTimer() {
  ...
  for (auto& type_expirations : expirations_) {
    ...
    for (const auto& timed_expirations : time_expirations_map) {
