...
void IPFSHostResolver::Resolve(const net::HostPortPair& host,
  ...
  receiver_.set_disconnect_handler(base::BindOnce(
      &IPFSHostResolver::OnMojoConnectionError, base::Unretained(this)));
}
...
void IPFSHostResolver::Reset() {
  resolving_host_.erase();
  resolved_callback_.Reset();

  if (reset_callback_for_testing_)
    std::move(reset_callback_for_testing_).Run();
}
...
void IPFSHostResolver::OnMojoConnectionError() {
  Reset();
}
