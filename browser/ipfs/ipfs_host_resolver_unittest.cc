...
class FakeHostResolverNoResponse : public FakeHostResolver {
 public:
  FakeHostResolverNoResponse() : FakeHostResolver(std::string()) {}
  ~FakeHostResolverNoResponse() override {}
...
TEST_F(IPFSHostResolverTest, RunSuccess) {
  ...
  ipfs_resolver.SetResetCallbackForTesting(run_loop.QuitClosure());
...
TEST_F(IPFSHostResolverTest, ResetOnReuse) {
  ...
  std::unique_ptr<FakeHostResolverNoResponse> fake_host_resolver_no_response(
      new FakeHostResolverNoResponse());
  network_context->SetHostResolver(std::move(fake_host_resolver_no_response));

  base::RunLoop first_reset_loop;
  ipfs::IPFSHostResolver ipfs_resolver(network_context);
  ipfs_resolver.SetResetCallbackForTesting(first_reset_loop.QuitClosure());
  SetResolvedCallbackCalled(0);
  ...
  std::unique_ptr<FakeHostResolver> fake_host_resolver(
      new FakeHostResolver(first_host));
  fake_host_resolver->RespondTextResults(success);
  auto* fake_host_resolver_raw = fake_host_resolver.get();

  network_context->SetHostResolver(std::move(fake_host_resolver));
  fake_host_resolver_raw->SetExpectedHost(second_host);
  ipfs_resolver.Resolve(
  ...
  first_reset_loop.Run();

  base::RunLoop second_reset_loop;
  ipfs_resolver.SetResetCallbackForTesting(second_reset_loop.QuitClosure());
  ASSERT_TRUE(ipfs_resolver.IsRunning());
  EXPECT_EQ(ipfs_resolver.host(), second_host);
  second_reset_loop.Run();
