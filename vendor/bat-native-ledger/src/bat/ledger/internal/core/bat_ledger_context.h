...
// Convenience class for defining task components. Subclasses must implement
// a |Start(...)| method that begins the asynchronous operation.
template <typename T>
class BATLedgerTask : public BATLedgerContext::Component {
 ...
 protected:
  ...
  // Returns a OnceCallback for the specified non-static member function.
  template <typename C, typename... Args>
  auto Continuation(void (C::*fn)(Args...)) {
    return base::BindOnce(fn, base::AsWeakPtr(static_cast<C*>(this)));
  }
